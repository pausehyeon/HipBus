...
<script type="text/javascript">
	...
	$(document).ready(
			function() {
				...
				if (quest.getParameter("nextPageToken") != null) { // 이전페이지에서 넘어온 경우
					loadVideos("${channelid}", "${googleApiKey}", quest
							.getParameter("nextPageToken"));
				} else if (quest.getParameter("prevPageToken") != null) {// 다음 페이지에서 넘어온 경우
					loadVideos("${channelid}", "${googleApiKey}", quest
							.getParameter("prevPageToken"));
				} else {
	...
	function loadVideos(channel_id, googleApiKey, pageToken) {
		if (pageToken) {
			var params = "pageToken=" + pageToken + "&part=snippet&channelId="
					+ channel_id + "&key=" + googleApiKey;
		} else {
			var params = "part=snippet&channelId=" + channel_id + "&key="
					+ googleApiKey;
		}
		var request;
		request = new Request(
				function() {
					...
					if (request.httpRequest.readyState == 4) {
							...
							if (jsonData.pageInfo.totalResults != 0) { // 동영상이 있는경우
								...
								for (var i = 0; i < jsonData.items.length; i++) {
									...
									if (data.id.kind == 'youtube#video') {
										...
										video += '<p class="w3-center w3-tiny">${str_title}'
												+ data.snippet.channelTitle
												+ ' ${str_writeDate}'
												+ data.snippet.publishedAt
												+ '</p>';
										video += '<h5>' + data.snippet.title
												+ '</h5>';
										video += '</div>';
									}
								}
								if (jsonData.nextPageToken
										&& !jsonData.prevPageToken) {
									video += '<div class="w3-row w3-center">';
									video += '<div class="w3-col w3-padding-12">'
									video += '<a class="w3-btn w3-hover-red w3-round" href="crewBusVideos.do?driver=${driver}&nextPageToken='
											+ jsonData.nextPageToken
											+ '">▶</a>';
									video += '</div></div>';
								} else if (jsonData.prevPageToken
										&& !jsonData.nextPageToken) {
									video += '<div class="w3-row w3-center">';
									video += '<div class="w3-col w3-padding-12">'
									video += '<a class="w3-btn w3-hover-red w3-round" href="crewBusVideos.do?driver=${driver}&prevPageToken='
											+ jsonData.prevPageToken
											+ '">◀</a>';
									video += '</div></div>';
								} else if (jsonData.nextPageToken
										&& jsonData.prevPageToken) {
									video += '<div class="w3-row w3-center">';
									video += '<div class="w3-col w3-padding-12">'
									video += '<a class="w3-btn w3-hover-red w3-round" href="crewBusVideos.do?driver=${driver}&prevPageToken='
						...
						$('#console')
								.html(
										"<img src='/HipBus/view/img/loading.gif' alt='로딩 중입니다' style='width:100px'>");
					}
				}, "https://www.googleapis.com/youtube/v3/search", "GET",
				params);
		request.sendRequest();
	}
//-->
			...
			<div class="w3-col m7">
				<div class="w3-container w3-card-2 w3-white w3-round w3-margin-bottom w3-margin-left w3-margin-right w3-padding-32 w3-padding-xxlarge">
					<div class="w3-container w3-center w3-black">
						<h3>
							<i class="fa fa-list-ul"></i>&nbsp; ${crewname}${str_myVideos}
						</h3>
					</div>

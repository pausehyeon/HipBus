...
<body class="w3-theme-l5" onload="myBusSet()">
	...
	<script type="text/javascript">
		...
		function makeBoard(data) {
			...
			modBtn += '<button type="button" id="modComplete" class="w3-btn w3-theme-l1" style="display:none" onclick="modComplete('
					+ data.num + ')">';
			...
			if (data.email == "${sessionScope.memEmail}"
					|| "${driver}" == "${sessionScope.memEmail}"
					|| "${my_level}" == 3) { // 작성자와 세션이메일이 같거나 버스주인과 이메일이 같으면 버튼을 붙인다
				board += appendBtn;
			}
			board += '</div>';

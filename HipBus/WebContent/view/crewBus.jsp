...
<script type="text/javascript">
						...
						if (request.httpRequest.status == 200) {
							...
							if (code == "inserted") {
								...
								if (boardList.firstChild != null) {
									...
									boardList.insertBefore(newdiv,oldFirstChild);	// 첫번째 방명록 위에 새 방명록을 붙인다
								}
	...
	function modComplete(num) {
		var params = 'num=' + num + '&content='	+ $('#board_' + num + ' textarea').val();
		request = new Request(
				function() {
					if (request.httpRequest.readyState == 4) {
						if (request.httpRequest.status == 200) {
	...
	function makeBoard(data) {
		...
		board += '<textarea readonly="true" rows="3" class="w3-padding-left">'+ data.content+ "</textarea>";
			if(data.email=="${sessionScope.memEmail}" || "${isMember}"=='true' || "${my_level}"==3) {
				board += appendBtn;
			}
		board += '</div>';

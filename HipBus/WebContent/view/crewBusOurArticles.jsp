			...
			<div class="w3-col m7">
				<div class="w3-container w3-card-2 w3-white w3-round w3-margin-bottom w3-margin-left w3-margin-right w3-padding-32 w3-padding-xxlarge">
					<div class="w3-container w3-center w3-theme-l1">
						<h3>
							<i class="fa fa fa-align-left fa-fw w3-margin-right"></i>${crewname} ${str_myArticles}
						</h3>
					</div>
					<div class="w3-row">
						<!-- 글이없는경우 -->
					...
					<c:if test="${type == 1 and station!=null}">
						<div class="w3-center ">
							...
							<c:if test="${startPage > pageBlock}">
								<a class="w3-hover-black" href="crewBusOurArticles.do?driver=${driver}">◀◀</a>
								<a class="w3-hover-black" href="crewBusOurArticles.do?driver=${driver}&pageNum=${startPage-pageBlock}">◀</a>
							</c:if>
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								...
								<c:if test="${i != currentPage}">
									<a class="w3-hover-black" href="crewBusOurArticles.do?driver=${driver}&pageNum=${i}">[${i}]</a>
								</c:if>
							</c:forEach>
							<c:if test="${pageCount > endPage}">
								<a class="w3-hover-black" href="crewBusOurArticles.do?driver=${driver}&pageNum=${startPage+pageBlock}">▶</a>
								<a class="w3-hover-black" href="crewBusOurArticles.do?driver=${driver}&pageNum=${pageCount}">▶▶</a>
							</c:if>
						</div>
					</c:if>
				</div>
